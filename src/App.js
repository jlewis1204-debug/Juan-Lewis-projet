 ...
 * * NOTA: Este código incluye una "Autocarga de Estilos" para que se vea bien
 * automáticamente sin configurar nada más.
 */

import React, { useState, useEffect } from 'react';
import { 
  ShoppingBag, AlertCircle, CreditCard, 
  CheckCircle, Lock, Phone, Star, Droplet, 
  Calendar, Truck, MessageCircle,
  Settings, Edit2, ArrowLeft, Save, Trash2, Plus, User
} from 'lucide-react';

// --- AUTO-CARGA DE ESTILOS (Tailwind CSS) ---
// Esto hace que la app se vea bonita automáticamente
const useTailwind = () => {
  useEffect(() => {
...
const INITIAL_SERVICES = [
  { id: 'wash_fold', name_en: 'Wash & Fold (per lb)', name_es: 'Lavado y Doblado (por lb)', price: 1.5, icon: '🧺' },
  { id: 'dry_clean_shirt', name_en: 'Dry Clean Shirt', name_es: 'Lavado en Seco Camisa', price: 5, icon: '👔' },
  { id: 'dry_clean_suit', name_en: 'Dry Clean Suit', name_es: 'Lavado en Seco Traje', price: 15, icon: '🕴️' },
  { id: 'ironing', name_en: 'Ironing Service', name_es: 'Servicio de Planchado', price: 3, icon: '🔌' },
  { id: 'bedding', name_en: 'Bedding / Comforter', name_es: 'Ropa de Cama / Edredón', price: 20, icon: '🛏️' },
];
...
const mockDb = {
  onSnapshot: (q, cb) => {
      ...
      if (q?.id === 'config') {
        const defaultConfig = { phone: '609-000-0000', discountPercent: 5, zelleNumber: '609-000-0000', zelleMessage: 'Send screenshot via WhatsApp' };
        cb({ exists: () => true, data: () => local.config || defaultConfig });
      } else if (q?.id === 'services') {
        cb({ exists: () => true, data: () => ({ list: (local.services && local.services.length > 0) ? local.services : INITIAL_SERVICES }) });
  ...
  updateDoc: async (ref, data) => {
      const local = getLocalData();
      const updatedOrders = local.orders.map(o => o.id === ref.id ? { ...o, ...data } : o);
...
const ServiceEditor = ({ services, setServices, t }) => {
  ...
  const addNew = () => setLocalServices([...localServices, { id: Date.now().toString(), name_es: 'Nuevo', name_en: 'New', price: 0, icon: '✨' }]);
        ...
        {localServices.map((s) => (
          <div key={s.id} className="grid grid-cols-1 md:grid-cols-12 gap-2 items-center bg-gray-50 p-3 rounded border border-gray-200">
            ...
            <div className="md:col-span-4">
                <input className="w-full p-2 border rounded text-xs" placeholder="ES Name" value={s.name_es} onChange={e => updateField(s.id, 'name_es', e.target.value)} />
            </div>
            <div className="md:col-span-4">
                <input className="w-full p-2 border rounded text-xs" placeholder="EN Name" value={s.name_en} onChange={e => updateField(s.id, 'name_en', e.target.value)} />
            </div>
            <div className="md:col-span-2">
                <input type="number" className="w-full p-2 border rounded font-bold text-green-700" value={s.price} onChange={e => updateField(s.id, 'price', e.target.value)} />
            </div>
            <div className="md:col-span-1 text-center flex justify-center">
                <button onClick={() => remove(s.id)} className="text-white bg-red-500 hover:bg-red-600 p-2 rounded w-8 h-8 flex items-center justify-center"><Trash2 className="w-4 h-4"/></button>
            </div>
          </div>
        ))}
      </div>
...
const SettingsPanel = ({ config, setConfig, t }) => {
    ...
    const saveSettings = async () => {
        setSaveStatus('saving');
        const finalConfig = { ...editConfig, discountPercent: parseFloat(editConfig.discountPercent) || 0 };
    ...
    return (
        <div className="bg-white p-8 rounded-xl shadow-lg max-w-2xl mx-auto border border-gray-100">
            ...
            <div className="bg-green-50 p-4 rounded-lg border border-green-200 mb-6">
                 ...
                 <p className="text-xs text-green-600 mb-2">Number where you receive WhatsApp orders.</p>
            ...
            <div className="space-y-6">
              ...
              <div className="p-5 bg-purple-50 rounded-xl border-2 border-purple-100">
                  <h4 className="font-bold text-purple-900 mb-4 flex items-center"><CreditCard className="w-5 h-5 mr-2"/> {t.zelleConf}</h4>
                  <label className="block text-sm font-bold text-purple-800 mb-1">Zelle Number / Email</label>
                  <input value={editConfig.zelleNumber || ''} onChange={(e) => setEditConfig({ ...editConfig, zelleNumber: e.target.value })} className="w-full p-3 border border-purple-200 rounded-lg bg-white" placeholder="Ej: user@email.com" />
              </div>
            </div>
...
const AdminPanel = ({ user, setView, t, config, setConfig, services, setServices }) => {
  ...
  useEffect(() => {
    if (!user) return; 
    const unsub = db.onSnapshot({id: 'orders_query'}, (snap) => setOrders(snap.docs.map((doc) => ({ id: doc.id, ...doc.data() }))));
    return () => unsub();
  }, [user]);
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4 font-sans">
        <div className="bg-white p-8 rounded-2xl shadow-xl text-center max-w-sm w-full">
          ...
          <form onSubmit={handleLogin}>
            <input type="password" value={pinInput} onChange={(e) => setPinInput(e.target.value)} placeholder="PIN (1234)" className="w-full p-4 border-2 rounded-xl mb-4 text-center text-2xl outline-none focus:border-cyan-500" />
          ...
          <button onClick={() => setView('home')} className="mt-6 text-gray-500 font-bold flex items-center justify-center w-full hover:text-cyan-600">
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-10 font-sans">
      <div className="bg-cyan-800 text-white p-4 sticky top-0 z-50 shadow-md flex justify-between items-center">
          ...
          <button onClick={() => setView('home')} className="bg-white/20 hover:bg-white/30 px-4 py-2 rounded-lg font-bold text-sm flex items-center transition">
            <ArrowLeft className="w-4 h-4 mr-2"/> {t.back}
          </button>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          <button onClick={() => setTab('orders')} className={`px-6 py-3 rounded-xl font-bold transition flex items-center ${tab === 'orders' ? 'bg-cyan-600 text-white shadow-lg' : 'bg-white text-gray-600 hover:bg-gray-100'}`}>
            📦 {t.adminOrders} 
            {orders.length > 0 && <span className="ml-2 bg-red-500 text-white text-xs px-2 py-1 rounded-full">{orders.length}</span>}
          </button>
          <button onClick={() => setTab('services')} className={`px-6 py-3 rounded-xl font-bold transition ${tab === 'services' ? 'bg-cyan-600 text-white shadow-lg' : 'bg-white text-gray-600 hover:bg-gray-100'}`}>
            🏷️ {t.adminServices}
          </button>
          <button onClick={() => setTab('settings')} className={`px-6 py-3 rounded-xl font-bold transition ${tab === 'settings' ? 'bg-cyan-600 text-white shadow-lg' : 'bg-white text-gray-600 hover:bg-gray-100'}`}>
            ⚙️ {t.adminSettings}
          </button>
        </div>
          ...
          <>
            <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-blue-500 text-white p-4 rounded-xl shadow-lg">
                    <div className="text-blue-100 text-sm font-bold uppercase">{t.totalOrders}</div>
                    <div className="text-3xl font-black">{orders.length}</div>
                </div>
                <div className="bg-green-500 text-white p-4 rounded-xl shadow-lg">
                    <div className="text-green-100 text-sm font-bold uppercase">{t.totalRevenue}</div>
              ...
              {orders.map((order) => (
                <div key={order.id} className="bg-white p-6 rounded-xl shadow border border-gray-100 relative overflow-hidden">
                  ...
                  <div className="flex flex-col md:flex-row justify-between mb-4 mt-2">
                    <div>
                      <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${order.status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}`}>
                      ...
                      <div className="text-sm text-gray-700 mt-3 grid grid-cols-2 gap-2 max-w-md">
                          <div className="bg-blue-50 p-2 rounded border border-blue-100">
                              <p className="text-xs text-blue-800 font-bold uppercase mb-1">Pickup</p>
                              <p>{order.details?.pickupDate}</p>
                              <p>{order.details?.pickupTime}</p>
                          </div>
                          <div className="bg-green-50 p-2 rounded border border-green-100">
                              <p className="text-xs text-green-800 font-bold uppercase mb-1">Delivery</p>
                    ...
                    <div className="text-right mt-4 md:mt-0 flex flex-col justify-between items-end">
                      ...
                      <div className="flex gap-2 mt-2">
                          <a href={getWhatsApp(order)} target="_blank" rel="noreferrer" className="inline-flex items-center text-green-600 font-bold text-sm bg-green-50 px-3 py-2 rounded-lg hover:bg-green-100">
                            <MessageCircle className="w-4 h-4 mr-2"/> Chat Customer
                          </a>
                          <button onClick={() => deleteOrder(order.id)} className="bg-red-100 text-red-600 p-2 rounded-lg hover:bg-red-200">
                  ...
                  <div className="bg-gray-50 p-3 rounded mb-4 text-sm border">
                    {Object.entries(order.items || {}).map(([k, v]) => (
                      <span key={k} className="mr-3 bg-white border px-2 py-1 rounded inline-block mb-1 font-bold">{v}x {k}</span>
                    ))}
                  </div>
                  
                  <div className="flex gap-2 overflow-x-auto pb-2">
                    {['confirmed', 'cleaning', 'delivering', 'completed'].map((st) => (
                      <button key={st} onClick={() => updateStatus(order.id, st)} disabled={order.status === st}
                        className={`px-4 py-2 rounded text-sm font-bold border transition ${order.status === st ? 'bg-cyan-600 text-white' : 'bg-white hover:bg-gray-50'}`}>
                        {t.status[st] || st}
                      </button>
                    ))}
                  </div>
...
export default function App() {
  useTailwind(); // ACTIVAR ESTILOS AUTOMÁTICAMENTE
  ...
  const submitOrder = async (e) => {
    ...
    let orderData = {
      customer: { name: form.name, phone: form.phone, address: form.address, uid: user.uid },
      details: { ...form, zelleNumberUsed: config.zelleNumber },
      items: cart, allergies, aroma, express: isExpress, isMember,
      total: getTotal(),
      status: 'pending',
      createdAt: Date.now(),
    };
      ...
      <div className="min-h-screen flex flex-col items-center justify-center text-center p-6 bg-cyan-50 font-sans">
        <div className="bg-white p-10 rounded-3xl shadow-2xl max-w-md w-full animate-fade-in">
          ...
          <div className="bg-gray-100 p-4 rounded-xl mb-8 border-2 border-dashed border-gray-300">
              <p className="text-sm text-gray-500 uppercase font-bold">{t.orderNumberIs}</p>
              <p className="text-3xl font-mono font-black text-cyan-600">{lastOrder?.id}</p>
          </div>
          
          <a href={getOwnerWhatsApp()} target="_blank" rel="noreferrer" className="w-full bg-green-500 text-white py-4 px-6 rounded-xl font-bold hover:bg-green-600 transition flex items-center justify-center mb-4 shadow-lg transform hover:scale-105 animate-pulse">
            <MessageCircle className="w-8 h-8 mr-3"/> {t.sendWhastapp}
          </a>
          
          <button onClick={() => setView('home')} className="w-full bg-gray-100 text-gray-600 py-4 rounded-xl font-bold hover:bg-gray-200 transition flex items-center justify-center">
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-20 text-gray-800">
          ...
          <div className="flex justify-between h-20">
            <div className="flex items-center cursor-pointer" onClick={() => setView('home')}>
              <div className="bg-cyan-500 p-2 rounded-full mr-3 shadow-lg">
                <div className="h-8 w-8 text-white font-bold flex items-center justify-center text-xl">FW</div>
              </div>
              <div className="flex flex-col">
                <span className="font-extrabold text-2xl text-cyan-800 leading-none">Fast wave</span>
                <span className="text-xs text-cyan-500 tracking-widest uppercase font-bold">Laundry</span>
              </div>
            </div>
            <div className="flex items-center space-x-3">
              ...
              <select value={lang} onChange={(e) => setLang(e.target.value)} className="bg-gray-100 text-sm rounded-lg px-2 py-1 border-none outline-none cursor-pointer font-bold text-gray-700">
                <option value="en">🇺🇸 EN</option>
                <option value="es">🇪🇸 ES</option>
              </select>
              <button onClick={() => setView('cart')} className="relative p-3 text-gray-500 hover:text-cyan-600 transition">
              ...
              <button onClick={() => setView('admin')} className="text-xs text-gray-400 hover:text-cyan-800">
        ...
        <>
          <div className="relative h-[450px] flex items-center justify-center overflow-hidden bg-cyan-900">
            ...
            <div className="relative z-20 text-center px-4 max-w-4xl mx-auto">
              ...
              <h1 className="text-5xl md:text-7xl font-black text-white drop-shadow-lg mb-6 tracking-tight">{t.title}</h1>
              <p className="text-2xl md:text-3xl text-cyan-100 font-light mb-10 italic">"{t.heroSubtitle}"</p>
              <button onClick={() => document.getElementById('services')?.scrollIntoView({ behavior: 'smooth' })} className="group bg-white text-cyan-700 text-xl font-bold py-4 px-10 rounded-full shadow-2xl hover:scale-105 transition-all duration-300 flex items-center justify-center mx-auto">
          ...
          <div id="services" className="max-w-7xl mx-auto px-4 py-16">
            <div className="text-center mb-12">
              <h2 className="text-3xl font-black text-gray-900">{t.services}</h2>
              <div className="w-24 h-1.5 bg-cyan-500 mx-auto mt-4 rounded-full"></div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 mb-16">
              {services.map((s) => (
                <div key={s.id} className="bg-white p-6 rounded-2xl shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 border border-gray-100 flex flex-col justify-between">
                  <div>
                    <div className="text-5xl mb-4">{s.icon}</div>
                    <h3 className="font-bold text-lg mb-1">
                      {lang === 'es' ? s.name_es : s.name_en}
                    </h3>
                    <p className="text-2xl font-bold text-cyan-600">${s.price?.toFixed(2)}</p>
                  </div>
                  <div className="flex items-center justify-between bg-gray-50 p-2 rounded-xl mt-4">
                    <button onClick={() => updateCart(s.id, -1)} className="w-10 h-10 rounded-full bg-white shadow text-gray-400 hover:text-red-500 font-bold text-xl transition">-</button>
                    <span className="font-bold text-xl">{cart[s.id] || 0}</span>
                    <button onClick={() => updateCart(s.id, 1)} className="w-10 h-10 rounded-full bg-cyan-600 shadow text-white hover:bg-cyan-700 font-bold text-xl transition">+</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-8 mb-12">
              <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                <h3 className="font-bold text-xl text-gray-800 mb-6 flex items-center"><Droplet className="w-6 h-6 text-purple-500 mr-2" /> {t.preferredAroma}</h3>
                <div className="flex flex-wrap gap-3">
                  {AROMAS.map((a) => (
                    <button key={a} onClick={() => setAroma(a)} className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${aroma === a ? 'bg-purple-600 text-white shadow-lg scale-105' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{a}</button>
                  ))}
                </div>
              </div>
              <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                <h3 className="font-bold text-xl text-gray-800 mb-6 flex items-center"><AlertCircle className="w-6 h-6 text-red-500 mr-2" /> {t.productsToAvoid}</h3>
                <div className="grid grid-cols-2 gap-3">
                  {AVOID_PRODUCTS.map((p) => (
                    <label key={p.id} className={`flex items-center p-3 rounded-xl border-2 cursor-pointer transition-all ${allergies.includes(p.id) ? 'border-red-400 bg-red-50' : 'border-transparent bg-gray-50 hover:bg-gray-100'}`}>
                      <input type="checkbox" checked={allergies.includes(p.id)} onChange={() => setAllergies((x) => x.includes(p.id) ? x.filter((y) => y !== p.id) : [...x, p.id])} className="w-5 h-5 accent-red-500 mr-3" />
                      <span className="text-sm font-medium text-gray-700">{lang === 'es' ? p.label_es : p.label_en}</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            <div className="flex flex-col md:flex-row justify-center gap-6 mb-32">
              <label className={`flex items-center p-5 rounded-2xl border-2 cursor-pointer transition-all hover:shadow-md ${isExpress ? 'border-cyan-500 bg-cyan-50 ring-2 ring-cyan-200' : 'border-gray-200 bg-white'}`}>
                ...
                <div><span className="font-bold text-lg block text-gray-800">{t.express}</span><span className="text-sm text-cyan-700 font-bold">+20% Fee</span></div>
              </label>
              <label className={`flex items-center p-5 rounded-2xl border-2 cursor-pointer transition-all hover:shadow-md ${isMember ? 'border-yellow-400 bg-yellow-50 ring-2 ring-yellow-200' : 'border-gray-200 bg-white'}`}>
                ...
                <div><span className="font-bold text-lg block text-gray-800">{t.member}</span><span className="text-sm text-yellow-600 font-bold">{config.discountPercent}% {t.discountMsg}</span></div>
          ...
          {Object.keys(cart).length > 0 && (
            <div className="fixed bottom-6 left-0 right-0 px-4 flex justify-center z-40 animate-bounce-slow">
              <button onClick={() => setView('cart')} className="bg-gray-900 text-white w-full max-w-md py-4 px-8 rounded-full shadow-2xl flex justify-between items-center hover:scale-105 transition transform border-4 border-white/20 backdrop-blur-lg">
                <div className="flex items-center">
                  <span className="bg-cyan-500 text-white font-bold rounded-full w-8 h-8 flex items-center justify-center mr-3 shadow-lg">{Object.values(cart).reduce((a, b) => a + b, 0)}</span>
                  <span className="font-bold text-lg">{t.orderNow}</span>
                </div>
                <span className="font-mono text-2xl font-black tracking-tight">${getTotal().toFixed(2)}</span>
              </button>
            </div>
          )}
        </>
      ...
      {view === 'cart' && (
        <div className="max-w-2xl mx-auto px-4 py-12">
          <button onClick={() => setView('home')} className="mb-8 text-gray-500 font-bold hover:text-cyan-600 flex items-center transition bg-white px-4 py-2 rounded-lg shadow-sm">
            <ArrowLeft className="w-5 h-5 mr-2"/> {t.back}
          </button>
          ...
          <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 mb-8">
            ...
            {Object.keys(cart).length === 0 ? <p className="text-gray-400 text-center py-4">Carrito vacío.</p> : (
              <div className="space-y-3">
                {Object.entries(cart).map(([id, q]) => {
                  const s = services.find((x) => x.id === id);
                  return (
                    <div key={id} className="flex justify-between text-gray-700 border-b border-gray-50 pb-2">
                      <span><span className="font-bold text-gray-900">{q}x</span> {lang === 'es' ? s.name_es : s.name_en}</span>
                      <span className="font-bold">${(s.price * q).toFixed(2)}</span>
                    </div>
                  );
                })}
                ...
                <div className="flex justify-between items-center pt-6 mt-4 border-t-2 border-dashed border-gray-200">
                  ...
                  <span className="text-4xl font-black text-cyan-600">${getTotal().toFixed(2)}</span>
                </div>
              </div>
            )}
          </div>
          ...
          {Object.keys(cart).length > 0 && (
            <form onSubmit={submitOrder} className="space-y-6 bg-white p-8 rounded-3xl shadow-lg border border-gray-100">
              ...
              <div className="grid gap-4">
                ...
                <input required placeholder="Phone Number" type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="w-full p-4 bg-gray-50 rounded-xl border focus:bg-white focus:border-cyan-500 outline-none transition" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                  ...
                  <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                    ...
                    <div className="space-y-2">
                        <div>
                            <span className="text-xs text-blue-600 font-bold ml-1">{t.pickupDate}</span>
                            <input required type="date" value={form.pickupDate} onChange={(e) => setForm({ ...form, pickupDate: e.target.value })} className="w-full p-3 bg-white rounded-lg border border-blue-200 focus:border-blue-500" />
                        </div>
                        <div>
                            <span className="text-xs text-blue-600 font-bold ml-1">{t.pickupTime}</span>
                            <select className="w-full p-3 bg-white rounded-lg border border-blue-200 focus:border-blue-500" value={form.pickupTime} onChange={(e) => setForm({...form, pickupTime: e.target.value})}>
                  ...
                  <div className="bg-green-50 p-4 rounded-xl border border-green-100">
                    ...
                    <div className="space-y-2">
                        <div>
                            <span className="text-xs text-green-600 font-bold ml-1">{t.deliveryDate}</span>
                            <input required type="date" value={form.deliveryDate} onChange={(e) => setForm({ ...form, deliveryDate: e.target.value })} className="w-full p-3 bg-white rounded-lg border border-green-200 focus:border-green-500" />
                        </div>
                        <div>
                            <span className="text-xs text-green-600 font-bold ml-1">{t.deliveryTime}</span>
                            <select className="w-full p-3 bg-white rounded-lg border border-green-200 focus:border-green-500" value={form.deliveryTime} onChange={(e) => setForm({...form, deliveryTime: e.target.value})}>
                                {TIME_SLOTS.map(slot => <option key={slot} value={slot}>{slot}</option>)}
                            </select>
                        </div>
                    </div>
              ...
              <div className="pt-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <label className={`flex items-center p-4 rounded-xl border-2 cursor-pointer transition ${form.paymentMethod === 'cash' ? 'border-cyan-500 bg-cyan-50' : 'border-gray-200'}`}>
                    <input type="radio" name="payment" value="cash" checked={form.paymentMethod === 'cash'} onChange={() => setForm({ ...form, paymentMethod: 'cash' })} className="w-5 h-5 accent-cyan-600 mr-3" />
                    <div className="flex flex-col"><span className="font-bold text-gray-800">{t.payCash}</span><span className="text-xs text-gray-500">Cash / Card on site</span></div>
                  </label>
                  <label className={`flex items-center p-4 rounded-xl border-2 cursor-pointer transition ${form.paymentMethod === 'online' ? 'border-purple-500 bg-purple-50' : 'border-gray-200'}`}>
                    <input type="radio" name="payment" value="online" checked={form.paymentMethod === 'online'} onChange={() => setForm({ ...form, paymentMethod: 'online' })} className="w-5 h-5 accent-purple-600 mr-3" />
                    <div className="flex flex-col"><span className="font-bold text-purple-900">{t.payOnline}</span><span className="text-xs text-purple-600">Zelle Transfer</span></div>
                  </label>
                </div>

                {form.paymentMethod === 'online' && (
                  <div className="bg-purple-50 p-4 rounded-xl border border-purple-200 space-y-2 text-sm text-purple-900">
                    <p><strong>Zelle:</strong> {config.zelleNumber || config.phone}</p>
